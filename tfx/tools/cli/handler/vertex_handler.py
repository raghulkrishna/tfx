...
class VertexHandler(base_handler.BaseHandler):
  ...
  def create_run(self) -> None:
    ...
    # Use timestamp for uniqueness of the job_id
    job_id = '{}-{}'.format(pipeline_name, time.strftime('%Y%m%d%H%M%S'))
    logging.warn('job_id:%s', job_id)
    ...
    logging.warn('run executed.')
    job.wait_for_resource_creation()  # pytype: disable=attribute-error
    logging.warn('run waited.')

    click.echo('Run created for pipeline: ' + pipeline_name)
    job = pipeline_jobs.PipelineJob.get(resource_name=job_id)  # pytype: disable=attribute-error
    logging.warn('got job:%s.', job)
    self._print_run(job)
  ...
  def _print_run(self, run):
    ...
    data = [[
        run.name, run.state, run.create_time,
