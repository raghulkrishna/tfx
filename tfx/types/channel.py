...
class ComponentChannel:
  ...
  def __init__(self, type: Type[Artifact]):  # pylint: disable=redefined-builtin
    if not (inspect.isclass(type) and issubclass(type, Artifact)):  # pytype: disable=wrong-arg-types
      raise ValueError(
          'Argument "type" of ComponentChannel constructor must be a subclass of '
...
class Channel(json_utils.Jsonable, ComponentChannel):
  """Tfx Channel.
...
class InputUnionChannel(ComponentChannel):
  """Union of multiple Channels with the same type."""

  def __init__(self, type: Type[Artifact], input_channels: List[Channel]):  # pylint: disable=redefined-builtin
    super().__init__(type=type)
...
def union(input_channels: Iterable[Channel]) -> InputUnionChannel:
  ...
  return InputUnionChannel(input_channels[0].type, input_channels)
